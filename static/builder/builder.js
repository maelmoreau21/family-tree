...
function setupChart(payload) {
  ...
  // Custom Edit Panel Implementation
  function createEditPanel() {
    ...
    panel.className = 'edit-person-form' // Re-use existing class for styling
    panel.style.position = 'fixed'
    panel.style.top = '0'
    panel.style.right = '-400px' // Hidden by default
  ...
  async function handleImageUpload(file, personId) {
    ...
    setUploadFeedback('Téléversement...', 'saving')
    ...
    try {
      ...
      // Update person data
      const datum = dataArray.find(p => p.id === personId)
      if (datum) {
        datum.data[chartConfig.imageField || 'avatar'] = data.url
        chart.updateTree({ initial: false, tree_position: 'inherit' })
        scheduleAutoSave()
      ...
      setUploadFeedback('Photo mise à jour', 'success')
    } catch (e) {
      console.error(e)
  ...
  chart.on('click', (event) => {
    if (event.data && event.data.id) {
      const datum = dataArray.find(d => d.id === event.data.id)
      if (datum) openEditPanel(datum)
    }
  })
